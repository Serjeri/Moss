...
class Database():
    ...
    def insert_regist(self, register: RegistrationContent, social: SocialContent, utilities: UtilitiesContent, 
        ...
        with self.db:
            ...
            utilities.departmental_affiliation.set(" "),utilities.composition.set(" "),utilities.military_rank.delete(0,END),
            ...
            self.cursor.execute( '''INSERT INTO information(lastname,name,patronymic,date_of_birth,
                ...
                series,number,Social_data,Service_data,Medical_data,health_information,Discharge_data) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)''',(register.lastname.get(),
                ...
                social_id,utilities_id,medicine_id,health_id,discharge_id,))
    ...
    def select_map_m(self):
        with self.db:
            return self.cursor.execute(""" SELECT lastname, name, patronymic,date_of_birth,gender,category,
                        blood_type,rh_factor,information,exposure_dose,Chronic_diseases,Identity_document,series,number,
                        Service_data.Departmental_affiliation,Service_data.Composition,Service_data.Military_rank,
                        Service_data.Personal_number, Service_data.Position,Service_data.vus,
    ...
    def map_information(self,id):
        with self.db:
            idTab = """ SELECT health_information FROM information WHERE id = ? """
            idKey = self.cursor.execute(idTab,(id,))
            idForenKey = idKey.fetchone()
            print(idForenKey[0])
            return self.cursor.execute(""" SELECT * FROM health_information WHERE id = ? """,(idForenKey[0],))

    def medical_departure(self):
        with self.db:
            return self.cursor.execute(""" SELECT * FROM Discharge_data """)

    def conclusion_m(self):
        with self.db:
            return self.cursor.execute(""" SELECT lastname, name, patronymic, Medical_data.diagnosis,Service_data.Military_rank,
    ...
    def conclusion_s(self):
        with self.db:
            return self.cursor.execute(""" SELECT lastname, name, patronymic, Medical_data.diagnosis,Social_data.Category,Medical_data.date_of_receipt,
                                        Discharge_data.date_of_departure,Discharge_data.exodus
                                        FROM information
                                        LEFT JOIN Medical_data ON Medical_data.id = information.Medical_data
                                        LEFT JOIN Social_data ON Social_data.id = information.Social_data
